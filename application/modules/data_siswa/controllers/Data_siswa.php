...
class Data_siswa extends CI_Controller
{
    ...
    public function datatable()
    {
        ...
        foreach ($q_datanya as $d) {
            ...
            $link_aktif_user = $d['jml_aktif'] > 0 ? '' : '<a href="#" onclick="return aktifkan(\'' . $d['id'] . '\');" class="btn btn-xs btn-info"><i class="fad fa-user"></i> Aktifkan User</a>';

            $data_ok[3] = '<a href="' . base_url() . $this->d['url'] . '/edit/' . $d['id'] . '" class="btn btn-xs btn-success"><i class="fa fa-edit"></i> Edit</a>
                ' . $link_aktif_user . '
                <a href="#" onclick="return hapus(\'' . $d['id'] . '\');" class="btn btn-xs btn-danger"><i class="fad fa-trash"></i> Hapus</a> ';
